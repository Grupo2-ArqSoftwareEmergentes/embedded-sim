...
struct Incident {
    ...
    void print() const {
        ...
        if (alertId.length() > 0) Serial.printf("    Alert ID: %s\n", alertId.c_str());
        if (deviceId.length() > 0) Serial.printf("    Device ID: %s\n", deviceId.c_str());
        if (hardwareId.length() > 0) Serial.printf("    Hardware ID: %s\n", hardwareId.c_str());
        if (spaceId.length() > 0) Serial.printf("    Space ID: %s\n", spaceId.c_str());
        if (thresholdMetric.length() > 0) Serial.printf("    Threshold metric: %s\n", thresholdMetric.c_str());
        if (thresholdValue.length() > 0) Serial.printf("    Threshold value: %s\n", thresholdValue.c_str());
        if (actualValue.length() > 0) Serial.printf("    Actual value: %s\n", actualValue.c_str());
        if (message.length() > 0) Serial.printf("    Message: %s\n", message.c_str());
...
private:
    ...
    static String toStringValue(JsonVariantConst value) {
    ...
    String normalizeMetric(const String& metric) const {
        ...
        if (normalized == "temperature" || normalized == "temp") return "TEMP";
        if (normalized == "humidity" || normalized == "hum") return "HUMIDITY";
        if (normalized == "co2") return "CO2";
        if (normalized == "pm25" || normalized == "pm2_5" || normalized == "pm2.5") return "PM25";
        return metric;
    }

    bool sameMetric(const String& a, const String& b) const {
        return normalizeMetric(a) == normalizeMetric(b);
    }

    int metricIndex(const String& metric) const {
        const String normalized = normalizeMetric(metric);
        if (normalized == "CO2") return 0;
        if (normalized == "PM25") return 1;
        if (normalized == "TEMP") return 2;
        if (normalized == "HUMIDITY") return 3;
        return -1;
    }

    void populateIncidentFromEvent(Incident& incident, const JsonObjectConst& evt) {
        incident.alertId = evt["alert_id"] | "";
        incident.deviceId = evt["device_id"] | "";
        incident.hardwareId = evt["hardware_id"] | "";
        incident.spaceId = evt["space_id"] | "";
        incident.metric = evt["metric"] | "";
        incident.thresholdMetric = evt["threshold_metric"] | "";
        incident.thresholdValue = toStringValue(evt["threshold_value"]);
        incident.actualValue = toStringValue(evt["actual_value"]);
        incident.message = evt["message"] | "";
        incident.status = evt["status"] | "";
        incident.occurredAt = evt["occurred_at"] | "";
        incident.resolvedAt = evt["resolved_at"] | "";
    ...
    void queueAck(int incidentId, const String& status) {
        ...
        for (int i = 0; i < pendingAckCount; i++) {
            if (pendingAcks[i].active && pendingAcks[i].incidentId == incidentId && pendingAcks[i].status == status) {
...
bool pollIncidents() {
    ...
    if (httpCode == 200) {
        ...
        if (count > 0 && doc["events"].is<JsonArray>()) {
            ...
            bool metricHasActive[4] = {false, false, false, false};
            ...
            for (JsonObject evt : events) {
                ...
                if (status == "ACTIVE" && metricIdx >= 0) {
                    metricHasActive[metricIdx] = true;
                }
                ...
                if (status == "ACTIVE") {
                    if (existingIndex < 0) {
                        ...
                        for (int i = 0; i < incidentCount; i++) {
                            if (sameMetric(activeIncidents[i].metric, metric)) {
                                sameMetricIndex = i;
                                break;
                            }
                        }
                        ...
                        if (sameMetricIndex >= 0) {
                            Serial.printf("[IncidentManager] Replacing old %s incident #%d with new #%d\n", 
                                          normalizedMetric.c_str(), activeIncidents[sameMetricIndex].id, id);
                            ...
                            populateIncidentFromEvent(activeIncidents[sameMetricIndex], evt);
                            ...
                            Incident incident;
                            incident.id = id;
                            populateIncidentFromEvent(incident, evt);
                    ...
                    if (existingIndex >= 0) {
                        ...
                        if (sameMetricIndex >= 0) {
                            ...
                            if (onIncidentResolved) {
                                ...
                                dummyIncident.id = id;
                                populateIncidentFromEvent(dummyIncident, evt);
            ...
            for (int i = incidentCount - 1; i >= 0; i--) {
                ...
                int idx = metricIndex(activeIncidents[i].metric);
                if (idx >= 0) shouldBeActive = metricHasActive[idx];
                ...
                if (!shouldBeActive && activeIncidents[i].status == "ACTIVE") {
                    Serial.printf("[IncidentManager] Cleaning stale %s incident #%d (no longer active on server)\n", 
                                  normalizeMetric(activeIncidents[i].metric).c_str(), activeIncidents[i].id);
    ...
    int getMetricPriority(const String& metric) const {
        ...
        if (normalized == "CO2") return 1;
        if (normalized == "PM25") return 2;
        if (normalized == "TEMP") return 3;
        if (normalized == "HUMIDITY") return 4;
        return 5;
    }
